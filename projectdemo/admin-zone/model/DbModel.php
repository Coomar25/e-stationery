...
{
    $conn = db_connect();
    $sql = "select * from food_item where fid = " . $id . " Limit 1"  ;
    $result = $conn->query($sql);
    $conn->close();
     if ($result->num_rows > 0) {
...
function update_food_by_id($id,$foodname,$fdesc,$price,$target){
    $conn = db_connect();
    $stmt = $conn->prepare("update food_item set food_name = ? , f_desc = ? , price = ? , imgurl = ? where fid = ?");
    $stmt->bind_param('ssisi', $foodname,$fdesc,$price,$target,$id);
    $result = $stmt->execute();
    if ($result) {
        $stmt->close();
        $conn->close();
...
function add_new_food($foodname,$fdesc,$price,$target){
    $conn = db_connect();
    $stmt = $conn->prepare("INSERT INTO food_item(food_name,f_desc,price,imgurl) values(?,?,?,?)");
    $stmt->bind_param('ssis',$foodname,$fdesc,$price,$target);
    $result = $stmt->execute();
    if ($result) {
        $stmt->close();
        $conn->close();
...
function signup_new_user($name, $email, $password, $address, $phone, $target, $create_date) {
    $conn = db_connect();
    $stmt = $conn->prepare("INSERT INTO user (name,email,password,address,phone,img,create_date) values(?, ?, ?,?, ?, ?,?)");
    $stmt->bind_param('ssssssi', $name, $email, $password, $address, $phone, $target, $create_date);
    $result = $stmt->execute();
    if ($result) {
        $stmt->close();
        $conn->close();
...
function delete_food_by_id($id) {
    $conn = db_connect();
    $sql = "Delete from food_item where fid=$id";
    $conn->query($sql);
